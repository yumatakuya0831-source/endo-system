...
- フロントエンド: React / Next.js。VercelへGitHub連携で配備するPC中心のレスポンシブ画面。
- 永続化: ブラウザーのLocal Storage。デモ全体の状態を端末・ブラウザー単位で保存する。
...
| --- | --- | --- | --- | --- | --- | --- |
外部APIは使用しない。クライアントがLocal Storageキー `endo-estimate-demo-state-v1` を読み書きする。
...
Local Storageに最新状態を保存する。デモ版では履歴・バックアップUIなし。保存失敗時は画面上の編集中データを維持し、再保存可能とする。
...
旧D1データはVercel版へ移行しない。ロールバックはVercelで直前のデプロイへ戻す。ブラウザー内の既存Local Storageデータは同一キーを維持する。
...
| NFR-001〜NFR-002 | AC-015〜AC-016 | §5, §7, §9 | `app/lib/domain.ts`, `app/estimate-app.tsx` | 計算精度と端末内永続化 |
| NFR-008 | AC-029 | §1, §2, §9 | `package.json`, Next.js App Router | Vercel本番ビルド |
...
| --- | --- | --- | --- | --- | --- |
| ADR-001 | デモ永続化 | Local Storageに状態JSONを保存（旧D1設計を置換） | Vercelで追加サービスなしにデモを利用可能にする | 端末間共有・同時編集・サーバーバックアップ不可 | 承認済み（デモ） |
...
| ADR-005 | 配備方式 | vinext/Cloudflare構成をNext.js/Vercel構成へ変更 | Vercelで発生した404を解消する | Sites/D1用コードと設定を廃止 | 承認済み（デモ） |

## 12. 実装順序と引き継ぎ

1. Next.js構成・Local Storage保存・ドメイン計算を実装する。
2. ダッシュボードと見積一覧を実装する。
